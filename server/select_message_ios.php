...
if($status_tag == 6){
	...
	$count = $check["sum"];
		...
		if($count < 1){
				...
				$sqla = "select * from resume_deliver,intern where resume_deliver.user_id = '$user_id' and resume_deliver.intern_id = intern.sid and resume_deliver.is_delete = 0";
...
//	查询非全部
	...
	$sql_test = "select count(*) as sum from resume_deliver where status_tag = '$status_tag' and resume_deliver.user_id ='$user_id' and is_delete = 0";
	$checkquery = mysql_query($sql_test);
	$check = mysql_fetch_assoc($checkquery);
	$count = $check["sum"];
		//	判断查询结果是否为空
			if($count < 1){
				...
				$sqla = "select * from resume_deliver,intern where resume_deliver.user_id ='$user_id' and resume_deliver.intern_id = intern.sid and resume_deliver.is_delete = 0 and status_tag = '$status_tag'";
				$querya = mysql_query($sqla);
					while ($result = mysql_fetch_assoc($querya)){
						$arraya[] = array_merge($result);}
		//	给结果加上success标示
